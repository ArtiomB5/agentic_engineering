...
**Методологии разработки — это то, как вы структурируете работу AI-агента для получения качественного кода.** Без методологии AI с одинаковой скоростью генерирует архитектурно правильное решение и технический долг — и не чувствует разницы. Исследование ACM (2025) показало, что AI-агенты генерируют в 1,75 раза больше логических ошибок, чем код, написанный человеком. В этой главе разобраны три подхода — TDD, SDD и BDD, — доказавшие эффективность с AI-агентами, и конкретные приёмы, помогающие держать код надёжным, а процесс — предсказуемым.

### Почему методологии важнее с AI, чем без него
Без методологии разработчик замедляется на плохом коде — он чувствует сопротивление. AI не чувствует сопротивления. Агент с одинаковой скоростью и уверенностью генерирует архитектурно правильный код и технический долг.

Без методологии:  
  Человек → медленно пишет плохой код → замечает проблему → исправляет  
  AI-агент → быстро пишет плохой код → не замечает → накапливает долг  

С методологией:  
  AI-агент → следует структуре → качество встроено в процесс

### Три методологии, которые работают с AI-агентами
...
Включите хуки в настройках Cline: **Settings → Feature Settings → Enable Hooks ✓**
...
Следующие инструменты — сторонние решения, не входящие в Cline. Они могут использоваться как MCP-серверы или отдельные CLI-инструменты:
...
Cline загружает все `.md` и `.txt` файлы из `.clinerules/` рекурсивно (кроме поддиректорий `hooks/`, `workflows/`, `skills/`). При росте спека свыше 200 строк — разбивайте на отдельные файлы:
...
└─ «Быстрый прототип, требования неясны»
   └─ Без методологии (но с Cline Checkpoints)
```
...
| Прототип | Без методологии + Cline Checkpoints |
