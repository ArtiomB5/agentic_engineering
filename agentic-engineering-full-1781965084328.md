...
> **v0.2.0 — предварительная версия.** Содержание может изменяться, дополняться и уточняться.
...
- **[Приложение 0: Руководство по созданию правил для Cline](appendix-0.md)** — пошаговое руководство: когда создавать правила, как определять тип, карта категорий, дерево решений, правила написания и диагностика проблем.
- **[Приложение 1: Руководство по созданию скиллов для Cline](appendix-1.md)** — полное руководство: прогрессивная загрузка, структура SKILL.md, 8 правил написания, decision trees, пример nextjs-vercel-deploy, чеклист перед публикацией.
- **[Приложение 2: Руководство по созданию воркфлоу для Cline](appendix-2.md)** — полное руководство: как работают воркфлоу технически, анатомия файла, правила именования, карта категорий, дерево решений, правила написания, примеры, диагностика и чеклист.
- **[Приложение 3: Руководство по созданию плагинов для Cline CLI](appendix-3.md)** — полное руководство по проектированию, написанию, тестированию и публикации плагинов для Cline. Охватывает оба формата — одиночный файл и пакет с зависимостями.
- **[Приложение 4: Руководство по созданию хуков для Cline](appendix-4.md)** — полное руководство по двум системам хуков — файловым (`.cline/hooks/`) и плагинным runtime-хукам. Охватывает технику, форматы, примеры и диагностику.
...
Использование одной и той же модели для любых задач — самая распространённая ошибка в агентной разработке. Разные модели оптимизированы под разные сценарии, и фиксация на одной модели означает либо постоянную переплату за простые задачи, либо недостаточное качество на сложных.

В современных агентных средах доступны десятки провайдеров и сотни моделей. Правильный выбор экономит бюджет, снижает задержки и повышает качество результата.

**Ключевые факторы выбора модели:**

- **Скорость инференса (токенов в секунду).** Сверхбыстрые лёгкие модели идеальны для циклов итеративной разработки, быстрых правок и частых коротких запросов. Флагманские модели с глубоким рассуждением работают заметно медленнее, но их ответы точнее и глубже.
- **Качество рассуждений (reasoning).** Способность модели удерживать сложный многошаговый контекст, выявлять противоречия и строить архитектурные планы. Влияет на успешность решения задач высокой сложности.
- **Стоимость.** Разница в стоимости одной тысячи токенов между лёгкими и премиальными моделями может достигать 10–50 раз.
- **Контекстное окно.** Максимальный объём данных (кода, истории чата, документации), который модель может обработать за один сеанс. Современные модели имеют окно от 64 тысяч до 1 миллиона и более токенов.

**Принцип выбора модели по типу задачи:**

| Тип задачи | Рекомендация |
| :--- | :--- |
| Сложная архитектура, дизайн системы | Мощная рассуждающая (reasoning) модель |
| Рефакторинг большого кода | Модель с большим контекстным окном |
| Написание тестов и простая кодогенерация | Быстрая лёгкая или средняя модель |
| Исправление синтаксических ошибок | Сверхбыстрая лёгкая модель |
| Справочные вопросы и документация | Сверхбыстрая лёгкая модель |
| Сложный дебаггинг с множеством зависимостей | Мощная рассуждающая модель |
| Форматирование, переименование, работа с путями | Самая лёгкая доступная модель |

**Градация контекстных окон для различных сценариев:**

| Размер контекста | Сценарий использования |
| :--- | :--- |
| 64K токенов | Работа в рамках одного-двух файлов; короткие сессии; быстрые правки с минимальной историей чата |
| 128K токенов | Текущий рыночный стандарт: многофайловое редактирование в рамках одного модуля, средние сессии с умеренной историей чата |
| 200K токенов | Работа с несколькими связанными модулями; длительные сессии с богатой историей правок |
| 256–400K токенов | Крупные многомодульные проекты; рефакторинг с сохранением широкого контекста зависимостей |
| 1M+ токенов | Полный рефакторинг репозитория; глубокий архитектурный аудит; работа с большими наборами данных или объёмной документацией |

**Связь качества, скорости и возможностей:**
Лёгкие модели обеспечивают мгновенные ответы и минимальную стоимость, но могут пропускать тонкие логические ошибки. Средние модели (оптимальный выбор для 90% повседневных задач) дают наилучшее соотношение производительности и цены. Флагманские модели с глубокими рассуждениями предназначены только для тех случаев, где требуется максимальная глубина анализа и где средние модели уже не справляются.

**Авто-конденсация контекста:** многие агентные среды поддерживают автоматическое сжатие контекста (Auto-condense) — при приближении к лимиту окна история разговора автоматически сжимается, сохраняя ключевую информацию. Включите эту настройку при работе с большими проектами, чтобы избежать потери связи между частями кода.

---

## 4.7. Auto Model Avoidance — Избегание раздельных моделей для режимов

Многие агентные среды позволяют настроить разные модели для режимов планирования (Plan Mode) и исполнения (Act Mode). Отказ от этой возможности — распространённая ошибка: вы платите максимальную цену даже за те задачи, которые могла бы решить модель в десятки раз дешевле. Запрос «переименуй переменную» и запрос «спроектируй стратегию кэширования для распределённой системы» получают одну и ту же модель, хотя их сложность несопоставима.

**Ключевое ограничение Plan Mode:** в режиме планирования агент может читать файлы, выполнять поиск и обсуждать стратегию, но не может изменять файлы или выполнять команды. Это архитектурное ограничение, а не просто рекомендация — оно удерживает разговор в фокусе понимания задачи и не даёт перейти к реализации раньше времени.

История чата сохраняется при переключении режимов. Агент помнит всё, что обсуждалось в Plan Mode, поэтому при переходе в Act Mode не нужно повторять контекст — переключение бесшовное.

**Как включить раздельные модели:**

1. Открыть настройки Cline (Cline Settings).
2. Включить чекбокс "Use different models for Plan and Act modes".
3. Выбрать предпочтительную модель для каждого режима в появившихся вкладках Plan Mode / Act Mode.

После включения переключение в режим планирования автоматически активирует модель, настроенную для рассуждений и исследований. Переключение в режим исполнения — модель для реализации кода. Выбор сохраняется при обратном переключении.

**Примеры конфигураций Plan/Act Mode:**

| Цель работы | Plan Mode (Планирование) | Act Mode (Исполнение) |
| :--- | :--- | :--- |
| Экономия бюджета | Лёгкая быстрая модель | Быстрая код-модель |
| Максимальное качество | Мощная рассуждающая модель | Сбалансированная модель общего назначения |
| Скорость прототипирования | Сверхбыстрая лёгкая модель | Сверхбыстрая лёгкая модель |

**Выбор режима по масштабу задачи:**

| Масштаб задачи | Рекомендация |
| :--- | :--- |
| Мелкая (опечатка, простой фикс) | Сразу Act Mode |
| Средняя (новый эндпоинт, компонент) | Plan Mode → Act Mode |
| Крупная (многофайловый рефакторинг) | /deep-planning → Act Mode |

**Команда /deep-planning:** для крупных задач, требующих тщательного анализа, используйте slash-команду /deep-planning. Она запускает расширенную сессию планирования: систематическое исследование кодовой базы, выявление всех затронутых файлов и зависимостей, создание детального плана реализации и уточняющие вопросы перед началом работы. Промпт автоматически адаптируется под семейство используемой модели.

**Strict Plan Mode:** дополнительная настройка, которая запрещает редактирование файлов в Plan Mode на уровне системы — даже если модель попытается это сделать. Полезна для жёсткого разделения фаз планирования и исполнения.

**Когда раздельные модели особенно полезны:**

- Сложный алгоритм или выбор архитектуры → Мощная рассуждающая модель в Plan Mode.
- Очень большой контекст (более 30–50 файлов) → Модель с максимальным контекстным окном в Plan Mode.
- Рутинные правки, комментарии, тесты → Сверхбыстрая лёгкая модель в Act Mode.
- Вопросы о синтаксисе или API → Лёгкая модель в Plan Mode.

**Практическая стратегия:**
Назначьте в Plan Mode премиальную модель с глубокими рассуждениями — она будет анализировать требования и составлять план. Назначьте в Act Mode быструю, сбалансированную по стоимости модель — она будет реализовывать уже принятое решение. Если задача требует исследования — используйте Plan Mode. Если вы точно знаете, что нужно сделать — переключитесь в Act Mode с более экономной моделью.

---

## 4.8. Premium Model Waste — Трата мощной модели на простые задачи

Использование флагманской модели для задач, которые не требуют глубоких рассуждений, — прямая трата бюджета. Разница в стоимости между лёгкой и премиальной моделью достигает 10–50 раз, а качество ответа на простую задачу при этом практически не отличается.

**Типичные задачи, для которых флагманская модель избыточна:**

- Написание юнит-тестов по готовому интерфейсу.
- Добавление комментариев и документации к существующему коду.
- Форматирование, переименование переменных, работа с путями.
- Простая кодогенерация по шаблону (CRUD, геттеры/сеттеры, стандартные компоненты).
- Исправление синтаксических ошибок и опечаток.

Разница между лёгкой и флагманской моделью проявляется только в задачах, которые требуют удержания сложного контекста, многошагового планирования или анализа архитектуры. Для всего остального мощная модель не даёт преимущества — она просто стоит дороже.

**Экономический аргумент:**
Следуя правильному распределению, вы можете снизить затраты на использование моделей на 60–80%. Если 50% ваших задач — написание тестов, документации и простые правки, использование для них лёгкой модели сохранит значительную часть бюджета для тех сложных случаев, где действительно нужна максимальная производительность.

**Практическая стратегия:**
Резервируйте мощные модели только для задач, включающих многократные итерации, логические цепочки и креативные архитектурные решения. Используйте средние сбалансированные модели как основу для 90% ежедневной работы. Переключайтесь на лёгкую модель всякий раз, когда задача механическая или однозначная.

---

## 4.9. Premium Model for Lookup Questions — Мощная модель для справочных вопросов

Отдельный и очень распространённый случай расточительства — использование флагманской модели для справочных вопросов. Это короткие запросы без кода и архитектурного контекста, ответ на которые не требует никаких рассуждений.

**Признаки справочного вопроса:**

- Промпт умещается в одну строку.
- Не содержит кода, файлов или описания архитектуры.
- Ответ — это факт, определение или объяснение синтаксиса.

**Типичные примеры:**

- "what is useState?" — справочный вопрос об API.
- "explain this error" — объяснение сообщения об ошибке.
- "add a comment" или "format this" — механические задачи.
- "what does this function do?" — объяснение простого факта.
- "what is the difference between == and ===" — вопрос о синтаксисе.

50 символов — это уже очень короткий промпт: "what does this error mean?" (27 символов), "fix the null check" (18 символов). Если запрос настолько короткий, что не требует контекста проекта, то и ответ на него не требует глубоких рассуждений.

**Почему это важно:**
Флагманская модель не даёт более точного ответа на вопрос «что такое Promise?», чем лёгкая. Она просто обрабатывает его в 10–50 раз дороже и медленнее. Накопленный эффект от таких запросов в течение рабочего дня может составлять значительную долю общих затрат.

**Практическая стратегия:**
Установите ментальное правило: если промпт умещается в одну строку и не содержит кода или архитектурного контекста — используйте сверхбыструю лёгкую модель. Мощная модель нужна только тогда, когда задача требует удержания сложного контекста, многошагового планирования или анализа архитектуры.
...
> **Важно:** Поддиректории `.cline/rules/workflows/`, `.cline/hooks/` и `.cline/rules/skills/` зарезервированы для соответствующих механизмов Cline (см. §5.4, §5.5, §5.2). Файлы внутри них не загружаются как правила. Размещайте файлы правил только непосредственно в `.cline/rules/`.
...
**Важное ограничение: VS Code Multi-Root Workspaces.** Если вы открываете монорепозиторий как multi-root workspace (несколько папок в одном окне VS Code), `.cline/rules` работает только в первой (primary) папке. Обходной путь: используйте глобальные правила (`~/Documents/Cline/Rules/`) — они применяются везде. Или убедитесь, что корень монорепозитория стоит первым в списке папок workspace.
...
- **Проектные** — `.cline/hooks/` — версионируются вместе с кодом.
...
# Приложение 0: Руководство по созданию правил для Cline
